...
OPS_LOG_HUB_URL=
OPS_LOG_HUB_KEY=
OPS_LOG_PROJECT=discordbot-auto-reaction
...
5. ops-log-hub 設定がある場合、起動完了イベントを送信する
...
- `config_error`: 起動時の拡張読み込み、slash command 同期、DB キャッシュ初期化の失敗
...
起動時に MongoDB から全設定を読み込み、メモリキャッシュに展開します。
